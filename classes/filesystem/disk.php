...
class DiskFileSystem extends RunnerFileSystem {
	 ...
	 * This parameter is ignored when readign the file or file info.
	...
	public function saveUploadedFile( $uploadFile, $userFilename ) {
		...
		$filename = DiskFileSystem::uniqueFilename( simplify_file_name( $userFilename ), $path );
		if( !$filename ) {
	...
	public function saveData( $data, $userFilename ) {
		...
		$filename = DiskFileSystem::uniqueFilename( $userFilename, $path );
	 ...
	 */
	static public function uniqueFilename( $filename, $path ) {
		...
		$atempt = 0;
		do { 
			$filename = $name."_".generatePassword(8).$extension;
			$file = $path.$filename; 
