...
class RunnerFileHandler {
	...
	protected function acceptUploadedFile( $uploadedFile ) {
		...
		if( isImageType( $file["type"] ) ) {
			...
			if( $img_width && $img_height && $this->pSet->getResizeOnUpload( $this->field ) ) {
	...
	protected function getTableRecord( $keys ) {
		$dc = new DsCommand();
		if( Security::fieldIsUserpic( $this->table(), $this->field ) ) {
	...
	protected function trimFilename( $name ) {
		...
		//	TODO - add extension based on the file type??
	...
	public static function getFileArray( $value, $field, $pSet ) {
		...
		$fileData = array();
		$fileData["usrName"] = basename( $value, "" );
